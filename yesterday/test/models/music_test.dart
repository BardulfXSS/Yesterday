...
void main() {
  group('Song thumbnail', () {
    test('returns default thumbnail if no albumArtUrl', () {
      final NetworkImage img = Song((b) => b
        ..title = 'foo'
        ..duration = Duration()).thumbnail.image;
      final NetworkImage defaultImg = Album.defaultThumbnail.image;
      ...
      testWidgets('shows album art given albumArtUrl', (t) async {
        mockNetworkImagesFor(() async {
          ...
          await t.pumpWidget(shell<String>(
            child: Artist((b) => b
              ..name = 'foo'
              ..albums.add(Album((b) => b
                ..name = 'bar'
                ..coverArtUrl = url))).thumbnail,
          ));
          await t.pumpAndSettle();
      ...
      testWidgets('shows 4x4 grid of album art covers', (t) async {
        mockNetworkImagesFor(() async {
          ...
          await t.pumpWidget(shell<String>(
            child: Artist((b) => b
              ..name = 'foo'
              ..albums.addAll(urls.map((u) => Album((b) => b
                ..name = 'bar'
                ..coverArtUrl = u)))).thumbnail,
          ));
          await t.pumpAndSettle();
  ...
  group('Album thumbnail', () {
    test('returns default thumbnail if no albumArtUrl', () {
      final NetworkImage img = Album((b) => b.name = 'foo').thumbnail.image;
      final NetworkImage defaultImg = Album.defaultThumbnail.image;
    ...
    test('returns network image from albumArtUrl', () {
      final url = 'https://i.pinimg.com/originals/da/18/3e/'
          'da183e46c3de1eaefa4f6705ca9a50dd.jpg';
      final NetworkImage img = Album((b) => b
        ..name = 'foo'
        ..coverArtUrl = url).thumbnail.image;

      expect(img.url, url);
    });
  });
  group('Playlist thumbnail', () {
    group('given 0 songs with albumArtUrls', () {
      testWidgets('shows default image', (t) async {
        mockNetworkImagesFor(() async {
          await t.pumpWidget(shell<String>(
